      ...
      return (
        <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
          <SheetContent
            ...
            data-mobile="true"
            className={cn(`w-[--sidebar-width] p-0 text-sidebar-foreground [&>button]:hidden`, color === 'black' ? 'bg-black' : 'bg-off-white')}
            style={
              {
...
>(({ className, username, children, ...props }, ref) => {
  const { open } = useSidebar();
  const [showName, setShowName] = React.useState(open);

  React.useEffect(() => {
    if (open) {
      const timeOut = setTimeout(() => setShowName(true), 300);
      return () => clearTimeout(timeOut);
    } else {
      setShowName(false);
    }
...
>(({ className, asChild = false, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      className={cn(
        'flex h-5 shrink-0 items-center rounded-md p-3 font-medium text-off-white outline-none ring-sidebar-ring transition-[margin,opa] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <li
      ...
      className={cn('group/menu-item relative', open && 'px-5', className)}
      {...props}
    />
  );
});
