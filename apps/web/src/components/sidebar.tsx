                ...
                <SidebarBody className="justify-between gap-2">
                    <div className="flex flex-col flex-1 overflow-y-auto overflow-x-hidden">
                        ...
                        <div className="mb-11 px-1 flex gap-3 justify-start items-center">
                            <Image src={avatar || "/profileplaceholder.png"} width={33} height={33} alt="avatar" className="rounded-full"/>
                            {open && <p className="text-white text-xl font-medium">{name}</p>}
                        </div>
                </SidebarBody>
            </SidebarConfig>
            {children}
            {/* {isOpen && <div className="fixed inset-0 z-40 text-3xl text-neutral-300 bg-neutral-950/50">test</div>} */}
        </div>
    );
}
export const Logo = () => {
